...
describe('Cities API', () => {
  ...
  describe('POST /cities', () => {
    it('should add a new city to the monitoring list', async () => {
      ...
      expect(res.body.message).toBe(`${cityName} has been added to the monitoring list.`);
    });
  ...
  describe('DELETE /cities/:city', () => {
    it('should remove a city from the monitoring list', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.message).toBe(`${cityName} has been removed from the monitoring list.`);
    });
    ...
    it('should return 404 if city is not found in the monitoring list', async () => {
      ...
      expect(res.status).toBe(404);
      expect(res.body.message).toBe(`${cityName} is not found in the monitoring list.`);
    });
  });
});
