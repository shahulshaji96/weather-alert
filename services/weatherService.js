...
const fetchAndStoreWeather = async (city) => {
  ...
  try {
    ...
    if (!data || !data.name || !data.main || !data.main.temp || !data.weather || !data.weather[0].main) {
    ...
    logger.info(`Weather data successfully fetched and stored for city: ${city}`);
    return data;
  } catch (error) {
    await transaction.rollback();
    logger.error(`Error occurred while processing weather data for city: ${city}`, { error: error.message, stack: error.stack });
    throw error;
  }
};
